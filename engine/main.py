...
async def handle_check_packages(req_id: str, params: dict) -> None:
    """Check which pip packages are installed. params: {packages: ["pkg1", "pkg2"]}"""
    import importlib
    ...
    for pkg in packages:
        try:
            ...
            result[pkg] = False
    send_result(req_id, {"installed": result})


async def handle_install_package(req_id: str, params: dict) -> None:
    """Install a pip package. params: {package: "pkg_name"}"""
    import subprocess
    pkg = params.get("package", "")
    if not pkg:
        send_error(req_id, "Missing 'package' parameter")
        return
    send_event("install_status", {"package": pkg, "status": "installing"})
    try:
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "-q", pkg],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        )
        send_event("install_status", {"package": pkg, "status": "done"})
        send_result(req_id, {"package": pkg, "success": True})
    except subprocess.CalledProcessError as e:
        send_event("install_status", {"package": pkg, "status": "error"})
        send_result(req_id, {"package": pkg, "success": False, "error": str(e)})
